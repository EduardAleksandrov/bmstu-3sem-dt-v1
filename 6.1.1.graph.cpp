...
private:
    std::string *stack;
    std::string *stackBase, *stackPointer;
    int stack_size;
public:
    Stack(int s_size): stack_size{s_size}
    {
        try
        {
            stack = new std::string [stack_size + 1];
        }
    ...
    bool pushStack(std::string value)
    {
    ...
    bool popStack(std::string &upperElement) 
    {
    ...
    void getAllStack(std::vector<std::string>& all_stack)
    {
        int stackNum = stackPointer - stackBase;
        for(int i = 0; i < stackNum; i++)
...
int main()
{
    // bool check {0};
    // Stack one = Stack(10);
    // // check = one.pushStack("A");
    // // check = one.pushStack("B");
    // one.printStack();
    // std::string z;

    // std::vector<std::string> all_stack {};
    // one.getAllStack(all_stack);
    // for(int i = 0; i < all_stack.size(); i++)
    //     std::cout << all_stack[i] << " ";
    // std::cout << std::endl;
    
    // check = one.popStack(z);
    // if(check == 1)
    // {
    //     std::cout << "Стек пуст" << std::endl;
    // }
    ...
    while(true)
    {
        ...
        std::cout << upperElement << std::endl;
