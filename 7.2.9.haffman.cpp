...
int main()
{
    ...
    std::vector <Table_column> table {}; // таблица первая
    std::string file[] {"7.2.1.string.txt", "7.2.5.bits.dat"};
    
// Считываем из файла и заполняем таблицу
    wchar_t ch;
    std::wfstream fr(file[0], std::wios::in);
    if(!fr) {
...
// вычислили коды для каждого элемента
    std::vector <Binary_node> binary_node_table {}; // структура под каждый символ
...
// Запись в файл wchar
    ...
    std::wfstream in_file_wchar(file[1], std::wios::out | std::ios::binary);
    if(!in_file_wchar) {
    ...
    int32_t file_seekp_pointer_set = in_file_wchar.tellp();
    in_file_wchar.close();
...
// запись битов данных в бинарный файл
    ...
    wchar_t chh;
    std::wfstream frr(file[0], std::wios::in);
    if(!frr) {
        std::wcout << "Cannot open file!" << std::endl;
        return 1;
    }
    ...
    std::ofstream wf(file[1], std::ios::in | std::ios::ate | std::ios::binary);
    if(!wf) {
        std::wcout << "Cannot open file!" << std::endl;
        return 1;
    } 
...
// получение wchar из файла
    std::vector <Table_column> new_table {}; // новая таблица полученная из файла
    std::wfstream out_file_wchar(file[1], std::wios::in | std::ios::binary);
    if(!out_file_wchar) {
        std::cout << "Cannot open file!" << std::endl;
        return 1;
    }
...
// чтение бинарных данных
    ...
    wchar_t first_el;
    std::wfstream f_read(file[1], std::wios::in | std::wios::binary);
    if(!f_read) {
        std::wcout << "Cannot open file!" << std::endl;
        return 1;
    }
    ...
    std::fstream wff_bits(file[1], std::ios::in | std::ios::binary);
    if(!wff_bits) {
        std::wcout << "Cannot open file!" << std::endl;
        return 1;
    }
    wff_bits.seekg(file_seekp_pointer_get, std::ios::beg);
    int32_t num_of_bites{0}; //Количество байтов битов после символов
    char count_bites;
    ...
    std::fstream wff(file[1], std::ios::in | std::ios::binary);
    if(!wff) {
        std::wcout << "Cannot open file!" << std::endl;
        return 1;
    }
    std::bitset<8> upper_read(0);
    wff.seekg(file_seekp_pointer_get, std::ios::beg);
    ...
    while(wff.read((char *) &upper_read, 1))
    {
        ...
        for(int i = 0; i < tmp.binary_number.size(); i++)
        {
            ...
            if(ind_bite>=1 && ind_bite<=first_el_int)
            {
